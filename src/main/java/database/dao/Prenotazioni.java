...
public class Prenotazioni extends Connessione {
	...
	private static final String TUTTE_LE_PRENOTAZIONI = "SELECT idPrenotazione, rel.idGruppo, nomeGruppo, prenotazione.idStanza, nomeStanza, idAnagrafica, numeroOspiti, dataArrivo, dataPartenza\r\n"
			+ "FROM ( \r\n"
	...
	public static int getNprenotazioniIdAnagrafica( String idAnagrafica ) {
		int ritorno = 0;
		try {
			ResultSet res = eseguiSelect( "SELECT count(idAnagrafica) FROM Prenotazioni WHERE idAnagrafica="
					+ idAnagrafica );
			res.next();
